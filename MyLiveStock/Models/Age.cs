...
namespace MyLiveStock.Models
{
    public class Age
    {
        ...
        public string GetAge()
        {
            ...
            {
                age = string.Format("{0} mois {1} jour(s)", this.Months.ToString(), this.Days);
            }

            if (this.Years == 0 && this.Months == 0 && this.Days != 0)
            {
                age = string.Format("{0} jour(s)", this.Days.ToString());
            }

            if (this.Years == 0 && this.Months != 0 && this.Days == 0)
            {
                age = string.Format("{0} mois", this.Months.ToString());
            }
